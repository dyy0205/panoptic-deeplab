...
# Demo code.
# Example command:
# python dyy_tools/demo.py --cfg PATH_TO_CONFIG_FILE \
...
def main():
    ...
    try:
        with torch.no_grad():
            for i, fname in enumerate(sorted(input_list)):
                ...
                # save_annotation(semantic_pred, semantic_out_dir, name,
                ...
                save_annotation(pan_to_sem, semantic_out_dir, name,
                                add_colormap=True, colormap=meta_dataset.create_label_colormap(),
                                image=raw_image if args.merge_image else None)
                ins_id = panoptic_pred % meta_dataset.label_divisor
                pan_to_ins = panoptic_pred.copy()
                pan_to_ins[ins_id == 0] = 0
                save_instance_annotation(pan_to_ins, instance_out_dir, name,
                                         image=raw_image if args.merge_image else None)
                save_panoptic_annotation(panoptic_pred, panoptic_out_dir, name,
                                         label_divisor=meta_dataset.label_divisor,
